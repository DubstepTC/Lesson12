...
def computer_move():
    for n in range(3):
        if can_win(field[n][0], field[n][1], field[n][2], '0'):
            return
        if can_win(field[0][n], field[1][n], field[2][n], '0'):
            return
    if can_win(field[0][0], field[1][1], field[2][2], '0'):
        return
    if can_win(field[2][0], field[1][1], field[0][2], '0'):
        return
    for n in range(3):
...
for row in range(3):
    ...
    for col in range(3):
        ...
        line.append(button)
    field.append(line)
new_button = Button(win, text='new game', command=new_game)
